 ...
 * 进度条配置选项
 */
export interface EnhancedProgressBarOptions {
  /** 进度条标题 */
  title: string;
  /** 总数 */
  total: number;
...
export class EnhancedProgressBar {
  ...
  private options: Required<EnhancedProgressBarOptions>;
  private isActive: boolean = false;
  private supportsProgress: boolean;

  constructor(options: EnhancedProgressBarOptions) {
    this.options = {
      title: options.title,
      total: options.total,
 ...
 * 创建增强版进度条实例
 */
export function createEnhancedProgressBar(options: EnhancedProgressBarOptions): EnhancedProgressBar {
  return new EnhancedProgressBar(options);
}
